...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        #conv
        self.OWH = window_size[0] * window_size[1]
        self.IWH = self.OWH * 2
        self.conv = DWConv(self.OWH, self.OWH, 1, 1)
        self.conv1 = DWConv(self.IWH, self.OWH, 3, 1, 1)
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        x1 = torch.cat((qq, kk), dim=1)
        x1 = self.conv1(x1)
        x1 = torch.cat((x1, vv), dim=1)
        x1 = self.conv1(x1)
        x1 = self.conv(x1)
        x1 = x1.contiguous().view(B_, N, -1)
...
class SwinCBAMBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0., drop_path=0., conv_scale=0.01,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.conv_block = CBAM(c1=dim, ratio=16, kernel_size=7)
...
class SwinBCBBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=7, shift_size=0,
                 mlp_ratio=4., qkv_bias=True, drop=0., attn_drop=0., drop_path=0., conv_scale=0.01,
                 layer_scale_init_value=-1, n_win=7, qk_dim=None, qk_scale=None,
    ...
    def forward(self, x, attn_mask):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn1(x_windows)
        # x_windows = attn_windows.view(-1, self.window_size * self.window_size, C)  # [nW*B, Mh*Mw, C]
        # attn_windows = self.attn(x_windows, mask=attn_mask)  # [nW*B, Mh*Mw, C]
...
class SwinCBAM(nn.Module):
    ...
    def __init__(self, dim, c2, depth, num_heads, window_size, conv_scale,
